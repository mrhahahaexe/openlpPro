...
class SongSearchTabs(object):
    """
    An enumeration for the search tabs.
    """
    Search = 0
    Lyrics = 1
    Web = 2
...
class SongMediaItem(MediaManagerItem):
    ...
    def add_middle_header_bar(self):
        ...
        self.search_tab_bar.addTab(UiStrings().Search)
        self.search_tab_bar.addTab(translate('SongsPlugin.MediaItem', 'Lyrics'))
        self.search_tab_bar.addTab(translate('SongsPlugin.MediaItem', 'Web'))
        self.search_tab_bar.addTab(translate('SongsPlugin.MediaItem', 'Auto'))
        self.search_tab_bar.setObjectName('search_tab_bar')
        self.page_layout.insertWidget(1, self.search_tab_bar)
        # Create the lyrics tab widget
        self.lyrics_tab_widget = QtWidgets.QWidget(self)
        self.lyrics_tab_widget.setObjectName('lyrics_tab_widget')
        self.lyrics_layout = QtWidgets.QVBoxLayout(self.lyrics_tab_widget)
        self.lyrics_layout.setObjectName('lyrics_layout')
        self.lyrics_search_edit_layout = QtWidgets.QFormLayout()
        self.lyrics_search_edit_layout.setObjectName('lyrics_search_edit_layout')
        self.lyrics_search_label = QtWidgets.QLabel(self.lyrics_tab_widget)
        self.lyrics_search_label.setText(translate('SongsPlugin.MediaItem', 'Phrase:'))
        self.lyrics_search_edit = QtWidgets.QLineEdit(self.lyrics_tab_widget)
        self.lyrics_search_edit.setPlaceholderText(translate('SongsPlugin.MediaItem', 'Enter phrase from lyrics...'))
        self.lyrics_search_edit_layout.addRow(self.lyrics_search_label, self.lyrics_search_edit)
        self.lyrics_layout.addLayout(self.lyrics_search_edit_layout)
        self.lyrics_button_layout = QtWidgets.QHBoxLayout()
        self.lyrics_button_layout.setObjectName('lyrics_button_layout')
        self.lyrics_button_layout.addStretch()
        self.lyrics_search_button = QtWidgets.QPushButton(self.lyrics_tab_widget)
        self.lyrics_search_button.setIcon(UiIcons().search_lyrics)
        self.lyrics_search_button.setText(UiStrings().Search)
        self.lyrics_button_layout.addWidget(self.lyrics_search_button)
        self.lyrics_layout.addLayout(self.lyrics_button_layout)
        self.lyrics_tab_widget.hide()
        self.page_layout.insertWidget(3, self.lyrics_tab_widget)
        # Create the web tab widget
        self.web_tab_widget = QtWidgets.QWidget(self)
        self.web_tab_widget.setObjectName('web_tab_widget')
        self.web_layout = QtWidgets.QVBoxLayout(self.web_tab_widget)
        self.web_layout.setObjectName('web_layout')
        self.web_search_edit_layout = QtWidgets.QFormLayout()
        self.web_search_edit_layout.setObjectName('web_search_edit_layout')
        self.web_search_label = QtWidgets.QLabel(self.web_tab_widget)
        self.web_search_label.setText(translate('SongsPlugin.MediaItem', 'Song/Artist:'))
        self.web_search_edit = QtWidgets.QLineEdit(self.web_tab_widget)
        self.web_search_edit.setPlaceholderText(translate('SongsPlugin.MediaItem', 'Enter title or artist...'))
        self.web_search_edit_layout.addRow(self.web_search_label, self.web_search_edit)
        self.web_layout.addLayout(self.web_search_edit_layout)
        self.web_button_layout = QtWidgets.QHBoxLayout()
        self.web_button_layout.setObjectName('web_button_layout')
        self.web_button_layout.addStretch()
        self.web_search_button = QtWidgets.QPushButton(self.web_tab_widget)
        self.web_search_button.setIcon(UiIcons().search_text)
        self.web_search_button.setText(translate('SongsPlugin.MediaItem', 'Search Online'))
        self.web_button_layout.addWidget(self.web_search_button)
        self.web_layout.addLayout(self.web_button_layout)
        self.web_tab_widget.hide()
        self.page_layout.insertWidget(4, self.web_tab_widget)
        # Create the auto tab widget
        self.auto_tab_widget = QtWidgets.QWidget(self)
        self.auto_tab_widget.setObjectName('auto_tab_widget')
        self.auto_layout = QtWidgets.QVBoxLayout(self.auto_tab_widget)
        self.auto_layout.setObjectName('auto_layout')
        self.auto_status_label = QtWidgets.QLabel(self.auto_tab_widget)
        self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Idle'))
        self.auto_status_label.setStyleSheet('font-weight: bold;')
        self.auto_layout.addWidget(self.auto_status_label)
        self.auto_button_layout = QtWidgets.QHBoxLayout()
        self.auto_button_layout.setObjectName('auto_button_layout')
        self.auto_start_button = QtWidgets.QPushButton(self.auto_tab_widget)
        self.auto_start_button.setIcon(UiIcons().audio)
        self.auto_start_button.setText(translate('SongsPlugin.MediaItem', 'Start Recognition'))
        self.auto_button_layout.addWidget(self.auto_start_button)
        self.auto_complete_button = QtWidgets.QPushButton(self.auto_tab_widget)
        self.auto_complete_button.setIcon(UiIcons().search)
        self.auto_complete_button.setText(translate('SongsPlugin.MediaItem', 'Complete & Search'))
        self.auto_complete_button.setEnabled(False)
        self.auto_button_layout.addWidget(self.auto_complete_button)
        self.auto_layout.addLayout(self.auto_button_layout)
        self.auto_result_label = QtWidgets.QLabel(self.auto_tab_widget)
        self.auto_result_label.setText(translate('SongsPlugin.MediaItem', 'Results:'))
        self.auto_layout.addWidget(self.auto_result_label)
        self.auto_tab_widget.hide()
        self.page_layout.insertWidget(5, self.auto_tab_widget)
        # Signals and slots
        ...
        self.lyrics_search_edit.returnPressed.connect(self.on_lyrics_search_click)
        self.lyrics_search_button.clicked.connect(self.on_lyrics_search_click)
        self.web_search_edit.returnPressed.connect(self.on_web_search_click)
        self.web_search_button.clicked.connect(self.on_web_search_click)
        self.auto_start_button.clicked.connect(self.on_auto_start_click)
        self.auto_complete_button.clicked.connect(self.on_auto_complete_click)
    ...
    def on_focus(self):
        if self.search_tab_bar.currentIndex() == SongSearchTabs.Search:
            self.search_text_edit.setFocus()
            self.search_text_edit.selectAll()
        elif self.search_tab_bar.currentIndex() == SongSearchTabs.Lyrics:
            self.lyrics_search_edit.setFocus()
            self.lyrics_search_edit.selectAll()
        elif self.search_tab_bar.currentIndex() == SongSearchTabs.Web:
            self.web_search_edit.setFocus()
            self.web_search_edit.selectAll()
        elif self.search_tab_bar.currentIndex() == SongSearchTabs.Auto:
            self.auto_start_button.setFocus()
    ...
    def on_search_tab_bar_current_changed(self, index):
        """
        Handle tab bar switching.
        """
        self.search_widget.setVisible(index == SongSearchTabs.Search)
        self.lyrics_tab_widget.setVisible(index == SongSearchTabs.Lyrics)
        self.web_tab_widget.setVisible(index == SongSearchTabs.Web)
        self.auto_tab_widget.setVisible(index == SongSearchTabs.Auto)
        
        # Refresh the results list based on the new tab
        if index == SongSearchTabs.Search:
            if self.search_results:
                self.display_results_song(self.search_results)
        elif index == SongSearchTabs.Lyrics:
            if self.lyrics_results:
                self.display_results_song(self.lyrics_results)
        elif index == SongSearchTabs.Web:
            if self.web_results:
                self.display_results_web(self.web_results)
        elif index == SongSearchTabs.Auto:
            if self.auto_results:
                self.display_results_auto(self.auto_results)
        
        self.on_focus()

    def on_lyrics_search_click(self):
        """
        Perform a lyric phrase search.
        """
        search_keywords = self.lyrics_search_edit.text().strip()
        if not search_keywords:
            self.on_song_list_load()
            return
        log.debug('Lyrics Phrase Search: %s', search_keywords)
        self.application.set_busy_cursor()
        try:
            search_string = '%{text}%'.format(text=clean_string(search_keywords))
            filter_clauses = [Song.search_lyrics.like(search_string)]
            if self.favourite_toggle_button.isChecked():
                filter_clauses.append(Song.is_favourite.is_(True))
            search_results = self.plugin.manager.get_all_objects(Song, *filter_clauses)
            self.lyrics_results = search_results
            self.display_results_song(search_results)
        finally:
            self.application.set_normal_cursor()

    def on_web_search_click(self):
        """
        Perform a web search using Genius API.
        """
        search_keywords = self.web_search_edit.text().strip()
        if not search_keywords:
            return
        log.debug('Web Search: %s', search_keywords)
        self.application.set_busy_cursor()
        url = "https://genius-song-lyrics1.p.rapidapi.com/search/"
        querystring = {"q": search_keywords, "per_page": "10", "page": "1"}
        headers = {
            "x-rapidapi-key": os.environ.get('RAPIDAPI_KEY', 'a79b7c8c90mshecbe2dd9af4b899p146e84jsn3c7dbca0654d'),
        ...
        try:
            ...
            self.display_results_web(hits)
        except Exception as e:
            log.exception('Web Search failed')
            critical_error_message_box(translate('SongsPlugin.MediaItem', 'Web Search Error'),
                                       translate('SongsPlugin.MediaItem', 'Error searching online: {error}').format(error=e))
        finally:
            self.application.set_normal_cursor()

    def on_auto_start_click(self):
        """
        Trigger Google Assistant to find a song.
        """
        adb_path = os.path.join(AppLocation.get_directory(AppLocation.AppDir), '..', 'adb-tools', 'adb.exe')
        adb_path = os.path.normpath(adb_path)

        def run_adb(args):
            cmd = [adb_path] + args
            log.debug('Running adb: %s', ' '.join(cmd))
            return subprocess.run(cmd, capture_output=True, text=True, check=False)

        self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Opening Assistant...'))
        self.application.process_events()
        try:
            adb_offset = float(os.getenv('ADB_TIME_DURATION_IN_SEC', '0'))
        except (ValueError, TypeError):
            adb_offset = 0

        # Step 1: Start Voice Command
        run_adb(['shell', 'am', 'start', '-a', 'android.intent.action.VOICE_COMMAND'])
        time.sleep(max(0.1, 2.5 + adb_offset))

        # Step 2: Input "help me find a song"
        self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Requesting song search...'))
        self.application.process_events()
        run_adb(['shell', 'input', 'text', 'help%sme%sfind%sa%ssong'])
        time.sleep(max(0.1, 1.0 + adb_offset))

        # Step 3: Enter
        run_adb(['shell', 'input', 'keyevent', '66'])

        self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Listening/Detecting...'))
        self.auto_complete_button.setEnabled(True)
        self.auto_start_button.setText(translate('SongsPlugin.MediaItem', 'Restart Recognition'))

    def on_auto_complete_click(self):
        """
        Scrape results from Assistant and search for lyrics.
        """
        adb_path = os.path.join(AppLocation.get_directory(AppLocation.AppDir), '..', 'adb-tools', 'adb.exe')
        adb_path = os.path.normpath(adb_path)
        ...
        self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Scraping results...'))
        self.application.set_busy_cursor()
        self.application.process_events()
        try:
            adb_offset = float(os.getenv('ADB_TIME_DURATION_IN_SEC', '0'))
        ...
        try:
            ...
            if not os.path.exists(tesseract_path):
                log.error('Tesseract not found at: %s', tesseract_path)
                self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 
                    'Status: Error - Tesseract not found. Check .env path.'))
                return
            pytesseract.pytesseract.tesseract_cmd = tesseract_path
            
            # Ensure we have a fresh screenshot
            cache_dir = AppLocation.get_directory(AppLocation.CacheDir)
            if not os.path.exists(cache_dir):
                os.makedirs(cache_dir)
            screenshot_path = os.path.join(cache_dir, 'screen.png')
            ...
            time.sleep(max(0.1, 1.0 + adb_offset))  # Wait for capture and stabilize
            run_adb(['pull', '/sdcard/screen.png', screenshot_path])

            if not os.path.exists(screenshot_path):
                log.error('Failed to pull screenshot from device')
                self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Error - Could not pull screenshot'))
                return

            # Perform OCR
            self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Performing OCR...'))
            self.application.process_events()
            ...
            try:
                ...
                self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 
                    'Status: Error - Tesseract not installed/found.'))
                return
            except Exception as e:
                log.error('OCR Error: %s', str(e))
                self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 
                    'Status: Error during OCR processing.'))
                return
            log.debug('AutoRecognition OCR Text: %s', ocr_text)

            results = []
            lines = [line.strip() for line in ocr_text.split('\n') if line.strip()]
            
            # Refined OCR Parsing logic
            # Many layouts use: Topic Header (Sounds like...) -> List of [Artist \n Song] or [Song \n Artist]
            # Junk to ignore in the lines
            system_junk = [
                'assistant logo', 'assistant microphone', 'google assistant menu', 
                'what song is this?', 'help me find a song', 'lp me find a song',
                'unlock more assistant features', 'search', 'tap to dismiss',
                'google assistant container', 'assistant query text', 'assistant suggestion',
                'keyboard', 'assistant profiled avatar', 'search results', 
                'sounds like one of these', 'results', 'user avatar', 'dismiss assistant'
            ]
            ...
            log.debug('AutoRecognition Filtered OCR Lines: %s', filtered_lines)

            # Pattern 1: [Artist] \n [Song] (Common in "Sounds like..." cards)
            # Pattern 2: [Song] \n [Artist]
            # Pattern 3: "That's [Song] by [Artist]"
            
            for i, line in enumerate(filtered_lines):
                # Check for "That's ... by ..."
                if line.lower().startswith("that's ") and ' by ' in line.lower():
                    parts = line[7:].split(' by ')
                    if len(parts) >= 2:
                        results.append({'title': parts[0].strip(), 'artist': parts[1].strip()})
                        continue

                # Check for "by " prefix (common in single results)
                if line.lower().startswith('by ') and i > 0:
                    results.append({'title': filtered_lines[i-1], 'artist': line[3:].strip()})
                    continue

                # Check for "song by "
                if line.lower().startswith('song by ') and i > 0:
                    results.append({'title': filtered_lines[i-1], 'artist': line[8:].strip()})
                    continue

            # Fallback for pairs (Artist/Song or Song/Artist) if no results found yet
            if not results and len(filtered_lines) >= 2:
                # We'll assume the first two lines might be a match if they are reasonable length
                for j in range(0, len(filtered_lines) - 1, 2):
                    # In OCR, we often get Song \n Artist
                    results.append({'title': filtered_lines[j], 'artist': filtered_lines[j+1]})

            if not results:
                self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: No song identified (OCR)'))
                return
            ...
            if len(unique_results) == 1:
                title_artist = "{title} {artist}".format(title=unique_results[0]['title'], 
                                                        artist=unique_results[0]['artist'])
                self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Found! Searching...'))
                self.web_search_edit.setText(title_artist)
                self.on_web_search_click()
            else:
                self.auto_status_label.setText(translate('SongsPlugin.MediaItem', 'Status: Multiple matches found'))
                self.display_results_auto(unique_results)

        finally:
            self.application.set_normal_cursor()

    def display_results_auto(self, results):
        """
        Display multiple identified songs for user selection.
        """
        self.auto_results = results
        self.list_view.clear()
        for res in results:
            item_text = "{title} - {artist} [Identified]".format(title=res['title'], artist=res['artist'])
            item = QtWidgets.QListWidgetItem(item_text)
            # Store as a search string
            item.setData(QtCore.Qt.ItemDataRole.UserRole, res['title'] + " " + res['artist'])
            self.list_view.addItem(item)

    def display_results_web(self, hits):
        """
        Display web search results.
        """
        self.web_results = hits
        self.list_view.clear()
        for hit in hits:
            ...
            item_text = "{title} - {artist} [Web Result]".format(title=title, artist=artist)
            item = QtWidgets.QListWidgetItem(item_text)
            # Store ID as negative to indicate web result (Genius IDs are positive)
            item.setData(QtCore.Qt.ItemDataRole.UserRole, -int(genius_id))
            self.list_view.addItem(item)

    def fetch_and_create_temporary_song(self, genius_id):
        """
        Fetch lyrics from Genius and create a temporary song object.
        """
        log.debug('Fetching lyrics for Genius ID: %s', genius_id)
        url = "https://genius-song-lyrics1.p.rapidapi.com/song/lyrics/"
        querystring = {"id": str(genius_id)}
        headers = {
            "x-rapidapi-key": os.environ.get('RAPIDAPI_KEY', 'a79b7c8c90mshecbe2dd9af4b899p146e84jsn3c7dbca0654d'),
            "x-rapidapi-host": "genius-song-lyrics1.p.rapidapi.com"
        }
        try:
            response = requests.get(url, headers=headers, params=querystring, timeout=10)
            log.debug('Genius Lyrics Response Status: %s', response.status_code)
            response.raise_for_status()
            full_response = response.json()
            log.debug('Genius Lyrics Full Response: %s', full_response)
            
            # The structure seems to be either {"response": {"lyrics": ...}} or just {"lyrics": ...}
            data = full_response.get('response', {}).get('lyrics')
            if not data:
                data = full_response.get('lyrics')
            
            if not data:
                 log.error('No lyrics data found in response: %s', full_response)
                 return None
            
            lyrics_obj = data.get('lyrics', {})
            body = lyrics_obj.get('body', {})
            lyrics_plain = body.get('plain', '')
            
            # Fallback to HTML if plain is empty
            if not lyrics_plain and body.get('html'):
                log.debug('Plain lyrics empty, falling back to HTML')
                soup = BeautifulSoup(body.get('html'), 'lxml')
                # Replace <br> with newlines before getting text
                for br in soup.find_all("br"):
                    br.replace_with("\n")
                lyrics_plain = soup.get_text()
            
            tracking_data = data.get('tracking_data', {})
            title = tracking_data.get('title', 'Unknown Title')
            artist = tracking_data.get('primary_artist', 'Unknown Artist')
            
            log.debug('Extracted: Title="%s", Artist="%s", Lyrics Length=%d', title, artist, len(lyrics_plain))
            
            if not lyrics_plain:
                log.error('Lyrics body (plain) is empty!')
                return None
            song = Song()
            song.title = title
            song.search_title = clean_string(title)
            ...
            # Simple parsing of lyrics into OpenLP XML format
            # Genius lyrics often use [Verse], [Chorus] etc.
            song_xml = SongXML()
            current_verse = []
            verse_type = 'v'
            verse_label = '1'
            verse_counts = {'v': 0, 'c': 0, 'b': 0, 'p': 0, 'e': 0, 'o': 0, 'i': 0}

            lines = lyrics_plain.split('\n')
            for line in lines:
                line = line.strip()
                if not line:
                    continue
                if line.startswith('[') and line.endswith(']'):
                    if current_verse:
                        song_xml.add_verse_to_lyrics(verse_type, verse_label, '\n'.join(current_verse))
                        current_verse = []
                    ...
                    verse_counts[verse_type] += 1
                    verse_label = str(verse_counts[verse_type])
                else:
            ...
            # If no verses found, just dump everything into one verse
            if not list(song_xml.lyrics) and lyrics_plain:
                song_xml.add_verse_to_lyrics('v', '1', lyrics_plain)
                
            song.lyrics = song_xml.extract_xml().decode('utf-8')
            song.search_lyrics = clean_string(lyrics_plain)
            
            # Handle Author
            author = self.plugin.manager.get_object_filtered(Author, Author.display_name == artist)
            if not author:
                author = Author()
                author.display_name = artist
                author.first_name = ''
                author.last_name = artist
                self.plugin.manager.save_object(author)
            song.add_author(author)
            
            self.plugin.manager.save_object(song)
            return song
        except Exception as e:
            log.exception('Fetching lyrics failed')
            return None
    ...
    def generate_slide_data(self, service_item, *, item=None, context=ServiceItemContext.Service, **kwargs):
        ...
        if item_id is not None and isinstance(item_id, str):
            # This is an identified song from Auto tab
            self.search_tab_bar.setCurrentIndex(SongSearchTabs.Web)
            self.web_search_edit.setText(item_id)
            self.on_web_search_click()
            return False
        elif item_id is not None and isinstance(item_id, int) and item_id < 0:
            self.application.set_busy_cursor()
            song = self.fetch_and_create_temporary_song(abs(item_id))
            self.application.set_normal_cursor()
            if not song:
                critical_error_message_box(translate('SongsPlugin.MediaItem', 'Lyric Fetch Error'),
                                           translate('SongsPlugin.MediaItem', 'Could not fetch lyrics from the web.'))
                return False
            item_id = song.id
        else:
            song = self.plugin.manager.get_object(Song, item_id)
        ...
        if not (song.verse_order or '').strip():
