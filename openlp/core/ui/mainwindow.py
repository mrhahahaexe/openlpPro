...
from openlp.core.lib.audio_capture import AudioCaptureService
from openlp.core.lib.recognition import RecognitionService
...
class Ui_MainWindow(object):
    ...
    def setup_ui(self, main_window):
        ...
        # AI Sync Action
        self.ai_sync_action = create_action(main_window, 'ai_sync_action',
                                           icon=UiIcons().microphone,
                                           text=translate('OpenLP.MainWindow', 'AI Live Sync'),
                                           checked=False,
                                           checkable=True,
                                           category=translate('OpenLP.MainWindow', 'General'),
                                           triggers=main_window.on_ai_sync_toggle)
        add_actions(self.settings_menu, [None, self.ai_sync_action])
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow, LogMixin, RegistryProperties):
    ...
    def __init__(self):
        ...
        self.audio_capture_service = AudioCaptureService(self)
        self.recognition_service = RecognitionService(self)
        self.audio_capture_service.recording_finished.connect(self.recognition_service.process_audio)
        self.recognition_service.song_recognized.connect(self.on_ai_song_recognized)
        self.ai_sync_timer = QtCore.QTimer(self)
        self.ai_sync_timer.timeout.connect(self.on_ai_sync_tick)
    ...
    def on_ai_sync_toggle(self, checked):
        """
        Toggle the AI Live Sync feature.
        """
        if checked:
            self.log_info("AI Live Sync enabled")
            self.ai_sync_timer.start(15000)  # Start every 15 seconds
            self.on_ai_sync_tick()  # Trigger first one immediately
        else:
            self.log_info("AI Live Sync disabled")
            self.ai_sync_timer.stop()
            self.audio_capture_service.stop_recording()

    def on_ai_sync_tick(self):
        """
        Capture 10 seconds of audio for recognition.
        """
        if self.ai_sync_action.isChecked():
            self.audio_capture_service.start_recording(duration=10)

    def on_ai_song_recognized(self, title, artist):
        """
        Handle successful song recognition.
        """
        self.log_info(f"AI Sync finding song: {title} by {artist}")
        
        # 1. Access the Songs plugin media item
        songs_media_item = Registry().get('songs')
        if not songs_media_item:
            self.log_warning("Songs plugin not found in registry")
            return

        # 2. Trigger a search for the title
        songs_media_item.search_text_edit.setText(title)
        songs_media_item.on_search_text_button_clicked()

        # 3. If we have results, go live with the first one
        if songs_media_item.list_view.count() > 0:
            first_item = songs_media_item.list_view.item(0)
            songs_media_item.list_view.setCurrentItem(first_item)
            songs_media_item.on_live_click()
            self.show_status_message(f"AI Sync: Song '{title}' is now LIVE")
        else:
            self.information_message(translate('OpenLP.MainWindow', 'AI Sync Result'),
                                     f"Recognized '{title}', but it's not in your library.")
