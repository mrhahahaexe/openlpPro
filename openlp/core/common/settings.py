...
class Settings(QtCore.QSettings):
    ...
    * Adds functionality for OpenLP Portable. If the ``defaultFormat`` is set to ``IniFormat``, and the path to the Ini
    ...
    def value(self, key, defaultValue=None, type=None):
        """
        Override value to support environment variable overrides.
        Env vars format: OPENLP_SECTION_KEY (e.g. OPENLP_CORE_LANGUAGE)
        """
        env_key = 'OPENLP_' + key.upper().replace('/', '_').replace(' ', '_')
        if env_key in os.environ:
            env_val = os.environ[env_key]
            # Attempt type conversion based on default settings
            default_val = Settings.__default_settings__.get(key, defaultValue)
        ...
        return super().value(key, defaultValue, type=type) if type else super().value(key, defaultValue)
